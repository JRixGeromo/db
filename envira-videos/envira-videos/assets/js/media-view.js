...
wp.media.view.EnviraVideosItem = wp.Backbone.View.extend(
	{
		...
		updateItem: function( event ) {
			...
			if ( event.target.name == 'link' ) {
				...
				self.model.set( 'link', event.target.value, { silent: true } );

				// If no video link, revert the hosted_video flag
				if ( event.target.value == '' ) {

					self.model.set( 'hosted_video', false );
						...
						'envira_videos_is_hosted_video',
						{
							...
							success: function( response ) {
								if ( response ) {
									...
									self.model.set( 'hosted_video', true );
									self.model.view.$el.find( 'div.image' ).show();
								} else {
									// Not a self hosted video
									self.model.set( 'hosted_video', false );
									self.model.view.$el.find( 'div.image' ).hide();
								}
							},
							error: function( error_message ) {
								...
								if ( self.frame !== undefined ) {
									self.frame.content.get().trigger( 'loaded loaded:error', error_message );
								}
							}
							}
					);
				...
				// Refresh view
			if ( self.model.get( 'hosted_video' ) ) {
				self.model.view.$el.find( 'div.image' ).show();
			} else {
				self.model.view.$el.find( 'div.image' ).hide();
			}
...
wp.media.controller.EnviraVideos = wp.media.controller.State.extend(
	{
		...
		enviraVideosInsert: function() {
			...
			var videos    = [],
				validated = true,
				self      = this,
				frame     = self.frame.content.get();

			// Get toolbar button
			self.button = self.frame.toolbar.get().get( 'envira_videos_insert' );
			...
			if ( ! validated ) {
				...
				self.button.model.set( 'text', wp.media.view.l10n.insertIntoPost );
