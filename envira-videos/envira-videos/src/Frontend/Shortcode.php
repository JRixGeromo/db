...
class Shortcode {
	...
	public function init() {
		...
		add_filter( 'envira_gallery_output_single_item', array( $this, 'envira_gallery_container_css' ), 10, 5 );
			...
			foreach ( $album_data['galleries'] as $gallery_id => $gallery ) {
				...
				if ( ! empty( $gallery_data['gallery'] ) ) {
					foreach ( $gallery_data['gallery'] as $item_id => $item ) {
						if ( isset( $item['link'] ) && strpos( $item['link'], 'facebook/videos' ) !== false ) {
							$output_fb_script = true;
						}
					}
				}
	...
	/**
	 * Automatically add css class if an iframe (video in gallery) is going to be the content of the inner div.
	 *
	 * @since 1.0.0
	 *
	 ...
	 */
	public function envira_gallery_container_css( $output, $id, $item, $data, $i ) {

		if ( isset( $item['video_in_gallery'] ) && 1 === intval( $item['video_in_gallery'] ) ) {
			$output = str_replace( 'envira-gallery-item-inner', 'envira-gallery-item-inner has-iframe', $output );
		}
	...
	public function change_gallery_output_before_image( $output, $id, $item, $data, $i ) {
		...
		// If this is an instagram gallery, don't do anything.
	...
	public function change_gallery_image( $output, $id, $item, $data, $i ) {
		...
		if ( isset( $item['video'] ) && ( ! isset( $item['video_in_gallery'] ) || ! $item['video_in_gallery'] ) ) {
			if ( envira_get_config( 'videos_play_icon', $data ) ) {
				...
				if ( 'automatic' === $data['config']['layout'] || 0 === intval( $data['config']['columns'] ) || ( $data['config']['columns'] > 0 && ! $data['config']['lazy_loading'] ) ) {
